...
class VerifyEventActions(unittest.TestCase):
  ...
  def test_single_event_action(self):
    ...
    verify = verify_event_action(event_action)
    self.assertEquals(verify, [{STATE: [EVENT_ACTION_OFF]}])
  ...
  def test_two_event_action(self):
    event_action = {STATE: [EVENT_ACTION_OFF, EVENT_ACTION_ON]}
    verify = verify_event_action(event_action)
    self.assertEquals(verify, [{STATE: [EVENT_ACTION_OFF, EVENT_ACTION_ON]}])
  ...
  def test_any_single(self):
    event_action = {EVENT_PROPERTY_ANY: [EVENT_ACTION_OFF]}
    verify = verify_event_action(event_action)
    self.assertEquals(verify, [{EVENT_ACTION_ANY: [EVENT_ACTION_OFF]}])
...
class SubscribersTest(unittest.TestCase):
  ...
  def test_add_subscriber_any(self):
    ...
    self.assertDictEqual(s.subscriptions, {self.device: {EVENT_ACTION_ANY: {EVENT_ACTION_ANY: [self.app]}}})
  ...
  def test_add_subscriber_custom_any(self):
    ...
    """
    s = Subscriptions()
    s.add_subscriber(self.app, self.device, event_action={'CUSTOM': EVENT_ACTION_ANY})
    self.assertDictEqual(s.subscriptions, {self.device: {'CUSTOM': {EVENT_ACTION_ANY: [self.app]}}})
  ...
  def test_add_subscriber_any_custom(self):
    ...
    """
    s = Subscriptions()
    s.add_subscriber(self.app, self.device, event_action={EVENT_PROPERTY_ANY: 'CUSTOM'})
    s.add_subscriber(self.app, self.device, event_action={EVENT_PROPERTY_ANY: 'CUSTOM'})
    self.assertDictEqual(s.subscriptions, {self.device: {EVENT_PROPERTY_ANY: {'CUSTOM': [self.app]}}})
  ...
  def test_add_subscriber_multiple_event_action_same_property(self):
    ...
    self.assertDictEqual(s.subscriptions,
                         {self.device: {STATE: {EVENT_ACTION_ON: [self.app], EVENT_ACTION_OFF: [self.app]}}})

  def test_add_multiple_subscribers_any(self):
    s = Subscriptions()
    s.add_subscriber(self.app, self.device)
    s.add_subscriber(self.app_b, self.device)
    self.assertDictEqual(s.subscriptions, {self.device: {EVENT_ACTION_ANY: {EVENT_ACTION_ANY: [self.app, self.app_b]}}})
  ...
  def test_add_multiple_subscribers_different_apps(self):
    ...
    self.assertDictEqual(s.subscriptions, {
      self.device: {EVENT_ACTION_ANY: {EVENT_ACTION_ON: [self.app], EVENT_ACTION_OFF: [self.app_b]}}})
  ...
  def test_multiple_listen_to_any(self):
    ...
    self.assertDictEqual(s.subscriptions,
                         {self.device:   {EVENT_ACTION_ANY: {EVENT_ACTION_ANY: [self.app]}},
                          self.device_b: {EVENT_ACTION_ANY: {EVENT_ACTION_ANY: [self.app]}}})
  ...
  def test_get_subscriber_items(self):
    ...
    s.add_subscriber(self.app, self.device)
    s.add_subscriber(self.app_b, self.device, event_action=[EVENT_ACTION_OFF])
    self.assertDictEqual(s.subscriptions, {self.device: {
      EVENT_ACTION_ANY: {EVENT_ACTION_ON: [self.app], EVENT_ACTION_OFF: [self.app_b], EVENT_ACTION_ANY: [self.app]}}})
    items = s.get_subscriber_items(self.app, self.device)
    self.assertEqual(items, {EVENT_ACTION_ANY: {EVENT_ACTION_ON: True, EVENT_ACTION_ANY: True}})

  # DELETE SUBSCRIBER TESTS
  def test_delete_subscription_simple(self):
    s = Subscriptions()
    s.add_subscriber(self.app, self.device, event_action={STATE: [EVENT_ACTION_ON, EVENT_ACTION_OFF]})
    delete_count = s.delete_subscriber(self.app, self.device, event_action={STATE: [EVENT_ACTION_ON]})
    self.assertEqual(delete_count, 1)
    self.assertDictEqual(s.subscriptions, {self.device: {STATE: {EVENT_ACTION_ON: [], EVENT_ACTION_OFF: [self.app]}}})
  ...
  def test_delete_subscription_non_existant(self):
    s = Subscriptions()
    s.add_subscriber(self.app, self.device, event_action={STATE: [EVENT_ACTION_ON, EVENT_ACTION_OFF]})
    delete_count = s.delete_subscriber(self.app, self.device_b, event_action={STATE: [EVENT_ACTION_ON]})
    self.assertEqual(delete_count, 0)
    self.assertDictEqual(s.subscriptions,
                         {self.device: {STATE: {EVENT_ACTION_ON: [self.app], EVENT_ACTION_OFF: [self.app]}}})
  ...
  def test_change_parent_id(self):
    ...
    self.assertTrue(c)
    self.assertDictEqual(s.subscriptions,
                         {self.device_b: {STATE: {EVENT_ACTION_OFF: [self.app], EVENT_ACTION_ON: [self.app]}}})
  ...
  def test_change_subscriber_id(self):
    ...
    self.assertDictEqual(s.subscriptions,
                         {self.device: {STATE: {EVENT_ACTION_OFF: [self.app_b], EVENT_ACTION_ON: [self.app_b]}}})
  ...
  def test_change_subscriber_id_complex(self):
    ...
    self.assertDictEqual(s.subscriptions,
                         {self.device:   {STATE: {EVENT_ACTION_OFF: [self.app_b], EVENT_ACTION_ON: [self.app_b]}},
                          self.device_b: {EVENT_ACTION_ANY: {EVENT_ACTION_ANY: [self.app_b]}}})
  ...
  def test_change_subscriber_id_none_existant(self):
    ...
    self.assertDictEqual(s.subscriptions,
                         {self.device: {STATE: {EVENT_ACTION_OFF: [self.app], EVENT_ACTION_ON: [self.app]}}})

  def test_get_subscribers_any(self):
    s = Subscriptions()
    s.add_subscriber(self.app, self.device)
    subscribers = s.get_subscribers(self.device, {STATE: [EVENT_ACTION_ON]})
    self.assertListEqual(subscribers, [self.app])

  def test_get_subscribers_none(self):
    s = Subscriptions()
    s.add_subscriber(self.app, self.device)
    subscribers = s.get_subscribers(self.device_b, {STATE: [EVENT_ACTION_ON]})
  ...
  def test_get_subscribers_two(self):
    s = Subscriptions()
    s.add_subscriber(self.app, self.device, {STATE: [EVENT_ACTION_ANY]})
    s.add_subscriber(self.app_b, self.device, {STATE: [EVENT_ACTION_OFF]})
    subscribers = s.get_subscribers(self.device, {STATE: [EVENT_ACTION_ON]})
    self.assertListEqual(subscribers, [self.app])
  ...
  def test_get_subscribers_multiple_events(self):
    s = Subscriptions()
    s.add_subscriber(self.app, self.device, {STATE: [EVENT_ACTION_ON]})
    s.add_subscriber(self.app_b, self.device, {STATE: [EVENT_ACTION_OFF]})
    subscribers = s.get_subscribers(self.device, {STATE: [EVENT_ACTION_OFF, EVENT_ACTION_ON]})
    self.assertSetEqual(set(subscribers), {self.app, self.app_b})